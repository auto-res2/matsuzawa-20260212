...
def parse_et_cot_response(response: str) -> Tuple[Optional[Dict], Optional[str], Optional[float]]:
    """
    ...
    try:
        ...
        # Extract FINAL
        final_match = re.search(r"FINAL:\s*([-+]?[\d,]+\.?\d*)", response, re.IGNORECASE)
        if final_match:
...
def verify_et_cot_trace(vars_dict: Dict, trace_code: str, final_answer: float, question: str) -> Tuple[bool, Optional[str]]:
    """
    ...
    try:
        ...
        if abs(last_computed_value - final_answer) > 1e-6:
            return False, f"FINAL ({final_answer}) does not match last computed value ({last_computed_value})"
...
def run_inference(cfg: Dict) -> None:
        ...
        if prompt_type == "cot_baseline":
            ...
            if cfg["inference"].get("verification", {}).get("enabled", False):
                ...
                if not verification_passed and max_retries > 0:
                    ...
                    vars_dict, trace_code, predicted_answer = parse_et_cot_response(repair_response)
                    verification_passed, error_message = verify_et_cot_trace(
                        vars_dict, trace_code, predicted_answer, question
                    )
            else:
    ...
    verification_rate = sum(1 for r in results if r["verification_passed"]) / len(results)
...
def perform_sanity_validation(results: List[Dict], total: int) -> None:
    ...
    if total < 5:
        ...
        print(f"SANITY_VALIDATION_SUMMARY: {json.dumps({'samples': total, 'outputs_valid': 0, 'outputs_unique': 0})}")
        return
    ...
    if valid_outputs == 0:
        ...
        print(f"SANITY_VALIDATION_SUMMARY: {json.dumps({'samples': total, 'outputs_valid': valid_outputs, 'outputs_unique': 0})}")
        return
    
    # Outputs are not all identical
    unique_answers = len(set(r["predicted_answer"] for r in results if r["predicted_answer"] is not None))
    if unique_answers <= 1 and total > 1:
        print(f"SANITY_VALIDATION: FAIL reason=all_identical_outputs")
        print(f"SANITY_VALIDATION_SUMMARY: {json.dumps({'samples': total, 'outputs_valid': valid_outputs, 'outputs_unique': unique_answers})}")
        return
